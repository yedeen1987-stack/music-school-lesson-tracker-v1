...
- 老师和学生都有 Email。
- 支持课程前一天提醒和改课立即通知，发送结果写入 `email_logs`。
- 完成本节后通知学生和老师，并显示剩余课时。
- 支持自定义余额提醒阈值，例如 `10,7,5,3,1`。
- 提供每日 SQLite 备份脚本。
...
完成课程后，系统会立即发送“课程完成通知”，内容包含剩余课时。管理员可以在“基础设置”中修改余额提醒阈值，例如 `10,7,5,3,1`。当完成课程后余额刚好等于其中一个数字，系统会额外发送“课时余额提醒”。
